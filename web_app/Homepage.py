...
def main():
    ...
    if is_youtube_playlist(youtube_link):
        ...
        # Mode selection
        mode = st.radio("Select Question Answering Mode", modes,
                        on_change=clear_text)
        # Model selection based on the chosen mode
        if mode == 'Extractive':
            model = st.radio("Select an Extractive Model", extractive_models,
                             on_change=clear_text)
        elif mode == 'Abstractive':
            model = st.radio("Select an Abstractive Model", abstractive_models,
                             on_change=clear_text)
